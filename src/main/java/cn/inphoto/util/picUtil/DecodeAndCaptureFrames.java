...
/**
 * Takes a media container, finds the first video stream, decodes that stream, and then writes video frames at some
 * interval based on the video presentation time stamps.
 * 
 * @author trebor
 */
...
public class DecodeAndCaptureFrames
{
    ...
    /**
     * Write the video frame out to a PNG file every once and a while. The files are written out to the system's
     * temporary directory.
     * 
    ...
    /**
     * Takes a media container (file) as the first argument, opens it, reads through the file and captures video frames
     * periodically as specified by SECONDS_BETWEEN_FRAMES. The frames are written as PNG files into the system's
     * temporary directory.
     * 
